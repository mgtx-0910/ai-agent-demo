/**
 * Prompt 模板 - 进阶聊天篇：SystemMessagePromptTemplate / HumanMessagePromptTemplate
 * 
 * 将 system 和 human 模板分别定义为独立的 PromptTemplate 对象，
 * 再用 ChatPromptTemplate.fromMessages() 组合。相比 chat-prompt-template 的
 * 内联数组写法，这种方式模板复用性更强，可以在多个场景中共享同一段模板。
 * 
 * @see chat-prompt-template.mjs     — 基础版：内联数组写法
 * @see pipeline-prompt-template.mjs — 延伸：PipelinePromptTemplate 模块化组合
 */
