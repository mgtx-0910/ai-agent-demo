/**
 * 示例选择器 - 长度篇：LengthBasedExampleSelector 按 token 数自动截断
 * 
 * 大量示例中自动筛选出能放进 prompt 长度限制的子集，优先选短的示例。
 * 解决 FewShotPromptTemplate 手动指定示例时可能超出上下文窗口的问题。
 * 
 * @see example-selector2.mjs — 升级版：SemanticSimilarityExampleSelector 语义筛选
 * @see fewshot-prompt-template.mjs — 对比：手动指定示例的无选择器版本
 */
