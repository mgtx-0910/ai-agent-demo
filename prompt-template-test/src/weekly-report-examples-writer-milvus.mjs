/**
 * 数据准备：将周报示例写入 Milvus 向量库
 * 
 * 将 example-selector-*.json 中的周报示例经过 OpenAIEmbeddings 转向量后
 * 写入 Milvus。是 example-selector2.mjs 的前置步骤——语义选择器需要此数据才能检索。
 * 
 * @see example-selector2.mjs — 数据消费者：SemanticSimilarityExampleSelector 检索示例
 */
