/**
 * Pipeline 模板 - 复用篇：复用 pipeline-prompt-template 的模块写 OKR 邮件
 * 
 * 直接导入 pipeline-prompt-template 中的 personaPrompt 和 contextPrompt，
 * 替换 finalPrompt 为 OKR Review 场景。展示了 PipelinePromptTemplate 的核心价值：
 * 公共模块复用 + 业务模块替换，无需重复定义 persona 和 context。
 * 
 * @see pipeline-prompt-template.mjs  — 模块来源：personaPrompt / contextPrompt
 * @see pipeline-prompt-template3.mjs — 对比：ChatPromptTemplate 版 pipeline
 */
