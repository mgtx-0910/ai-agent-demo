/**
 * Pipeline 模板 - 基础篇：PipelinePromptTemplate 模块化组合
 * 
 * 将 prompt 拆解为多个子模块（persona/context/final），用 PipelinePromptTemplate 组装。
 * 每个子模块是独立的 PromptTemplate，可被多个业务场景复用。
 * 导出 personaPrompt、contextPrompt、pipelinePrompt 供其他文件引用。
 * 
 * @see pipeline-prompt-template2.mjs — 复用：相同模块写 OKR Review 邮件
 * @see pipeline-prompt-template3.mjs — 升级版：ChatPromptTemplate 对话格式
 * @see partial.mjs                  — 延伸：.partial() 预填变量
 */
