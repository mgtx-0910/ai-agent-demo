/**
 * Pipeline 模板 - 聊天格式篇：PipelinePromptTemplate + ChatPromptTemplate
 * 
 * 复用 pipeline-prompt-template 的子模块，但最终组装时使用 ChatPromptTemplate
 * 替代纯文本的 PipelinePromptTemplate，给 final prompt 添加 system 角色。
 * 适合需要区分对话角色的 pipeline 场景。
 * 
 * @see pipeline-prompt-template.mjs  — 模块来源：personaPrompt / contextPrompt
 * @see pipeline-prompt-template2.mjs — 对比：PipelinePromptTemplate 文本版
 */
