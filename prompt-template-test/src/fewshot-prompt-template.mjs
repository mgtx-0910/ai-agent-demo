/**
 * Few-shot 模板 - 基础篇：FewShotPromptTemplate 手动给示例
 * 
 * 在 prompt 中附上少量输入 → 输出示例，引导模型模仿示例格式回答。
 * 使用 FewShotPromptTemplate 组装 example_prompt + examples 数组，手动指定示例。
 * 
 * @see fewshot-chat-prompt-template.mjs — 升级版：聊天格式的 FewShotChatMessagePromptTemplate
 * @see example-selector1.mjs             — 进阶版：LengthBasedExampleSelector 自动选示例
 */
