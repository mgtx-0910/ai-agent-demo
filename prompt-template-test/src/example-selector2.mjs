/**
 * 示例选择器 - 语义篇：SemanticSimilarityExampleSelector + Milvus
 * 
 * 用向量相似度从 Milvus 中检索与当前输入最相关的示例，实现「根据问题自动选示例」。
 * 相比 LengthBasedExampleSelector 只考虑长度，语义选择器能选出更贴合的 few-shot 示例。
 * 
 * @see example-selector1.mjs                     — 对比：LengthBasedExampleSelector 按长度筛选
 * @see weekly-report-examples-writer-milvus.mjs  — 前置：将周报示例写入 Milvus 向量库
 */
