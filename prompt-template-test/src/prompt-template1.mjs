/**
 * Prompt 模板 - 基础篇：PromptTemplate 字符串模板
 * 
 * 使用 PromptTemplate.fromTemplate() 创建含 {变量} 占位符的字符串模板，
 * 通过 .format() 填充变量后直接作为 prompt 发送给模型。
 * 这是最基础的 prompt 模板用法。
 * 
 * @see chat-prompt-template.mjs  — 升级版：ChatPromptTemplate 区分 system/human 角色
 */
