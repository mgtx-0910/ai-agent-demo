/**
 * 多智能体 Supervisor（主管-下属）模式演示：
 * createSupervisor 生成一个「调度员」工作流，根据用户问题把任务分派给
 * weather_agent（天气）或 trivia_agent（城市小知识）这两个子代理，
 * 最后汇总输出并打印实际走过的节点路径。
 * 运行：node src/multi-agent-supervisor.mjs（需在 .env 配置模型相关变量）
 */
...
// 一条同时涉及「天气」和「小知识」的输入，考验调度员的拆分能力
const input = {
  messages: [
...
// 用 stream 流式收集：updates 模式记录每个节点的执行路径，values 模式拿最终状态
const nodePath = [];
let finalState = null;
