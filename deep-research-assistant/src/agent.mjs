// ============================================================================
// src/agent.mjs — Agent 工厂
//
// 构建「深度调研助手」的核心编排逻辑：
//   - 主 Agent（orchestrator）：规划任务、委派子 Agent、起草与定稿报告
//   - researcher 子 Agent：联网搜索单一子主题，产出 findings 文件
//   - editor 子 Agent：审阅草稿，只给反馈不改稿
//   - analyst 子 Agent：通过 QuickJS eval REPL 完成数值计算
//
// 使用 FilesystemBackend 将项目根目录作为虚拟文件系统根（virtualMode），
// Agent 通过 write_file / read_file 等工具在其中读写调研资料；
// 同时启用 AGENTS.md 长期记忆 与 /skills/ 技能目录。
// ============================================================================
...
// 项目根目录：以本文件（src/agent.mjs）的上一级为准
...
// 只负责一个聚焦子主题：可选 todo 拆解 -> 搜索（上限 3 次）-> 写 findings 文件 -> 结束。
// ----------------------------------------------------------------------------
const researcherSubAgent = {
  ...
  `,
  // 只暴露 web_search 一个工具，确保行为聚焦
  tools: [webSearch],
};

// ----------------------------------------------------------------------------
// editor（编辑）子 Agent
// 在草稿完成后被调用：只审阅不改稿，返回可操作的修改建议。
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// analyst（分析师）子 Agent
// 注入 createCodeInterpreterMiddleware，获得 eval REPL 能力，
// 让模型执行 JavaScript 完成数值计算与数据分析，而不是"凭空猜测"数字。
// ----------------------------------------------------------------------------
...
/**
 * 工厂函数：创建「深度调研助手」主 Agent。
 *
 * 依赖环境变量：
 *   - OPENAI_API_KEY   必填
 *   - OPENAI_MODEL     可选，默认 gpt-4o
 *   - OPENAI_BASE_URL  可选，兼容 OpenAI 协议的自定义网关地址
 *
 * @returns 组装好的 DeepAgent 实例
 */
export function createIntelligenceDeskAgent() {
  // 校验 API Key：缺失时直接抛错，避免模型调用失败后再排查
  ...
  // 虚拟文件系统后端：以项目根目录为 "/"，Agent 内部使用的均为虚拟路径
  const backend = new FilesystemBackend({
    rootDir: projectDir,
    virtualMode: true,
  });

  // 语言模型：temperature=0 保证调研输出的确定性
  ...
  // 可选：当底层模型接口不支持 profile.maxInputTokens 时，
  // 可取消注释，强制声明上下文窗口大小
...
// 导出项目根目录，供 CLI 定位 workspace 等路径使用
export { projectDir };
